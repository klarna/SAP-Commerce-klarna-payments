...
<%@ taglib prefix="klarnaaddress" tagdir="/WEB-INF/tags/addons/klarnapaymentaddon/responsive/address" %>
...
<c:choose>
	<c:when test="${is_klarna_exp_checkout}"> 
		...
		<form:form action="${paymentProcess}" name="klarnaExpCheckoutPaymentDetailsForm" id="klarnaExpCheckoutPaymentDetailsForm" modelAttribute="klarnaPaymentDetailsForm" method="post"> 
			<label class="kppayment-container"><input type="radio" name="paymentMethod" id="paymentMethod_klarna_exp_checkout" value=klarna_exp_checkout checked="checked" /> Klarna Express Checkout </label>
	...
	<c:otherwise>
		<c:if test="${is_klarna_active}">
			...
			<form:form>						
				...
				<label class="kppay	ment-container"><input type="radio" name="paymentMethod" id="paymentMethod_card" value="card" ${isCardSelected ? 'checked="checked"' : ''}/> Card </label>
