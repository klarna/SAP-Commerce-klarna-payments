...
import com.klarna.payment.facades.KlarnaSignInV1Facade;
 ...
 */
@Controller
@RequestMapping(value = "/klarna/signin/v1")
public class KlarnaSigninV1Controller extends AbstractPageController
{

	private static final Logger LOG = Logger.getLogger(KlarnaSigninController.class);

	@Resource(name = "klarnaSignInV1Facade")
	private KlarnaSignInV1Facade klarnaSignInV1Facade;
	...
	private static final String KLARNA_SIGNIN_CONSENT_URL = "/klarna/signin/v1/consent";
			...
			final HttpServletResponse response, final Model model, final RedirectAttributes redirectAttr)
	{
		...
		if (klarnaConfig != null && StringUtils.isNotBlank(klarnaConfig.getEnvironment()))
		{
			...
			{
				if (klarnaSignInV1Facade.validateSigninToken(klarnaSigninResponse, klarnaConfig.getEnvironment()))
				{

					profileStatus = klarnaSignInV1Facade.checkAndUpdateProfile(klarnaSigninResponse);
	...
	public String createCustomer(@ModelAttribute("klarnaSigninResponse") KlarnaSigninResponse klarnaSigninResponse,
			...
			final RedirectAttributes redirectAttr)
	{
		...
		try
		{
			klarnaSignInV1Facade.createNewCustomer(klarnaSigninResponse);
	...
	public String mergeAccount(@ModelAttribute("klarnaSigninResponse") KlarnaSigninResponse klarnaSigninResponse,
			final Model model, final HttpServletRequest request, final HttpServletResponse response,
			final RedirectAttributes redirectAttr)
	{
		String prevPage = sessionService.getAttribute("signInRefererPage");
				...
				&& StringUtils.isNotEmpty(klarnaSigninResponse.getUserAccountProfile().getEmail()))
		{
			try
			{
				...
				klarnaSignInV1Facade.updateCustomer(customer, klarnaSigninResponse.getUserAccountProfile(),
